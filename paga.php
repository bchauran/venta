<?php
	...
	$paypal_business = "sb-fvf2k220090@business.example.com";
	
	$paypal_currency = "USD";
	$paypal_cursymbol = "&usd";
	//$paypal_location = "MX";
	$paypal_returnurl = "http://localhost/venta/paypal/listo.php";
	$paypal_returntxt = "Pago Realizado Exitosamente!";
	$paypal_cancelurl = "http://localhost/venta/paypal/cancelado.php";
    ...
    //$cart=$_GET["array_con_codigos_paypal"];

    $cart=$_POST["array_con_codigos_paypal"];




    $_SESSION['Codigos_Carrito']=array();


    $_SESSION['Codigos_Carrito'] = $_POST["array_con_codigos_paypal"];
    

    $cantidad_decompra=$_POST["cantidad_comp"];


    //echo $cart."<br><br><br>";

    
    //$cart1 = $_GET["datospaypal"];

    //$cart = $_GET["querystr"];

    //$a=stripslashes($_GET[datospaypal]);
            

    //$cart=unserialize($cart1);

    //$cart = json_decode($_POST['array_con_codigos']);
	...
	$ppurl .= "&charset=utf-8&rm=2&upload=1";
	//$ppurl .= "&business=".$paypal_business;
	...
	foreach ($cart as $c) {
      ...
      /*
		...
		*/
		...
		$ppurl.="&item_name_$i=".urlencode($row["nompro"])."&quantity_$i=$r&amount_$i=".$row["precio"]."&item_number_$i=1";
